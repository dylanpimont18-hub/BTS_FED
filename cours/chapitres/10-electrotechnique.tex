...
% ============================================================
\section{Compensation du facteur de puissance}
% ============================================================

\subsection{Problématique du courant réactif en CVC}

Les moteurs asynchrones des pompes et ventilateurs consomment une puissance réactive importante ($\cos\varphi$ typique : 0,75--0,85). Un faible $\cos\varphi$ entraîne :
\begin{itemize}
  \item Un surcourant dans les câbles : pour la même puissance active, $I = P / (\sqrt{3} \times U \times \cos\varphi)$ augmente quand $\cos\varphi$ diminue.
  \item Des pertes Joule supplémentaires : $P_{Joule} = R \times I^2$.
  \item Une pénalisation tarifaire du fournisseur d'énergie (tarif HTA, seuil $\cos\varphi < 0{,}93$).
\end{itemize}

\subsection{Correction par condensateurs}

La puissance réactive capacitive $Q_C$ à installer pour relever $\cos\varphi$ de $\cos\varphi_1$ à $\cos\varphi_2$ :

\begin{equation}
  Q_C = P \times \left(\tan\varphi_1 - \tan\varphi_2\right) \quad [\text{kvar}]
\end{equation}

\begin{exemple}[title={Compensation du facteur de puissance — ventilateur CTA}]
  Ventilateur triphasé 400\,V, $P = \SI{7{,}5}{\kilo\watt}$, $\cos\varphi_1 = 0{,}78$.\\
  On souhaite atteindre $\cos\varphi_2 = 0{,}92$.

  \[
    \tan\varphi_1 = \tan(\arccos 0{,}78) = 0{,}802 \qquad \tan\varphi_2 = \tan(\arccos 0{,}92) = 0{,}426
  \]
  \[
    Q_C = 7{,}5 \times (0{,}802 - 0{,}426) = 7{,}5 \times 0{,}376 = \SI{2{,}82}{\text{kvar}}
  \]

  On installe un condensateur de \textbf{3\,kvar} (valeur normalisée au-dessus). Le courant absorbé passe de :
  \[
    I_1 = \frac{7\,500}{\sqrt{3} \times 400 \times 0{,}78} = \SI{13{,}9}{\ampere} \rightarrow I_2 = \frac{7\,500}{\sqrt{3} \times 400 \times 0{,}92} = \SI{11{,}8}{\ampere}
  \]
  Réduction de courant de $15\,\%$, soit une économie sur les pertes câbles de $(1 - 0{,}85^2) = 28\,\%$.
\end{exemple}
...
\begin{exemple}[title={Dimensionnement complet d'une armoire CVC — triphasé 400\,V}]
  \textbf{Données de l'installation :} armoire CVC d'une centrale de traitement d'air (tertiaire) comprenant :
  \begin{center}
    \begin{tabular}{lccc}
      \toprule
      Équipement & $P_n$ (kW) & $\cos\varphi$ & $I_n$ calc. (A) \\
      \midrule
      Ventilateur soufflage (1 unité) & 5{,}5 & 0{,}84 & $\frac{5500}{\sqrt{3}\times400\times0{,}84} = 9{,}45$ \\
      Ventilateur reprise (1 unité)   & 3{,}0 & 0{,}82 & $\frac{3000}{\sqrt{3}\times400\times0{,}82} = 5{,}29$ \\
      Pompe batterie chaude            & 1{,}5 & 0{,}80 & $\frac{1500}{\sqrt{3}\times400\times0{,}80} = 2{,}71$ \\
      Pompe batterie froide            & 1{,}5 & 0{,}80 & $2{,}71$ \\
      \bottomrule
    \end{tabular}
  \end{center}

  \textbf{Puissance active totale :}
  \[
    P_{tot} = 5{,}5 + 3{,}0 + 1{,}5 + 1{,}5 = \SI{11{,}5}{\kilo\watt}
  \]

  \textbf{Avec coefficient de foisonnement $k_f = 0{,}85$ :}
  \[
    P_{bilan} = 11{,}5 \times 0{,}85 = \SI{9{,}775}{\kilo\watt}
  \]

  \textbf{Puissance apparente totale} (cos$\varphi$ moyen pondéré $\approx 0{,}82$) :
  \[
    S_{tot} = \frac{9{,}775}{0{,}82} = \SI{11{,}92}{\text{kVA}}
  \]

  \textbf{Courant général absorbé :}
  \[
    I_{tot} = \frac{S_{tot}}{\sqrt{3} \times U} = \frac{11\,920}{\sqrt{3} \times 400} = \frac{11\,920}{693} = \SI{17{,}2}{\ampere}
  \]

  \textbf{Choix du disjoncteur général :} calibre normalisé au-dessus de 17,2\,A → \textbf{disjoncteur 20\,A courbe D} (charge moteurs, pouvoir de coupure 10\,kA minimum en BT tertiaire).

  \textbf{Vérification chute de tension} (câble d'alimentation 25\,m, section 4\,mm², cuivre) :
  \[
    \Delta U = \frac{\sqrt{3} \times 0{,}0225 \times 25 \times 17{,}2}{4} = \frac{16{,}75}{4} = \SI{4{,}19}{\volt}
  \]
  \[
    \Delta U\,\% = \frac{4{,}19}{400} \times 100 = 1{,}05\,\% \leq 3\,\% \quad \checkmark
  \]

  \textbf{Conclusion :} L'armoire CVC est dimensionnée avec un disjoncteur général 20\,A, des disjoncteurs moteurs individuels réglés au courant nominal de chaque équipement, et un câble 4\,mm² Cu conforme NF C 15-100.
\end{exemple}
...
% ============================================================
\section{Illustrations et diagrammes}
% ============================================================

\subsection{Triangle des puissances}

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=Stealth, thick, scale=1.2]
    % Vecteur S (hypoténuse)
    \draw[->, color=FedBlue, line width=2pt] (0,0) -- (4,0)
      node[midway, below=6pt, color=FedBlue, font=\bfseries] {$P$ (W)};
    % Vecteur Q (vertical)
    \draw[->, color=FedOrange, line width=2pt] (4,0) -- (4,2.5)
      node[midway, right=6pt, color=FedOrange, font=\bfseries] {$Q$ (var)};
    % Vecteur S (hypoténuse)
    \draw[->, color=FedRed, line width=2pt] (0,0) -- (4,2.5)
      node[midway, above left=2pt, color=FedRed, font=\bfseries] {$S$ (VA)};
    % Angle phi
    \draw[color=FedGreen, thin] (0.9,0) arc[start angle=0, end angle=32, radius=0.9];
    \node[color=FedGreen, font=\small] at (1.3,0.18) {$\varphi$};
    % Angle droit en bas à droite
    \draw[gray, thin] (3.85,0) -- (3.85,0.15) -- (4,0.15);
    % Formules
    \node[font=\small, align=left] at (6.8, 1.25) {%
      $S^2 = P^2 + Q^2$\\[4pt]
      $\cos\varphi = \dfrac{P}{S}$\\[6pt]
      $\tan\varphi = \dfrac{Q}{P}$};
  \end{tikzpicture}
  \caption{Triangle des puissances en courant alternatif — $P$ : puissance active (bleue), $Q$ : puissance réactive (orange), $S$ : puissance apparente (rouge), $\varphi$ : déphasage courant/tension (vert).}
  \label{fig:triangle-puissances}
\end{figure}

\subsection{Schéma unifilaire d'une armoire électrique CVC}

La figure~\ref{fig:armoire-cvc} représente l'architecture typique d'une armoire électrique CVC triphasée 400\,V.

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=Stealth, thick,
      boite/.style={draw, rectangle, minimum width=1.8cm, minimum height=0.7cm,
                    align=center, font=\small},
      depart/.style={draw, rectangle, minimum width=2.2cm, minimum height=0.7cm,
                     align=center, font=\scriptsize, fill=FedBlue!10},
      ligne/.style={draw, ->, color=FedBlue}]

    % Jeu de barres (arrivée réseau)
    \node[draw, fill=FedBlue!20, minimum width=8.5cm, minimum height=0.5cm,
          font=\small\bfseries, color=FedBlue] (jdb) at (0,0)
      {Jeu de barres triphasé L1 -- L2 -- L3 / N / PE \quad 400\,V 50\,Hz};

    % Disjoncteur général
    \node[boite, fill=FedOrange!15, below=0.8cm of jdb] (dg)
      {Disjoncteur général\\$Q_0$ — 32\,A cbe D\\Icc $\geq$ 10\,kA};
    \draw[ligne] (jdb.south) -- (dg.north);

    % Répartiteur interne
    \node[draw, fill=gray!10, minimum width=8.5cm, minimum height=0.4cm,
          font=\small, below=0.6cm of dg] (repartiteur)
      {Répartiteur interne — départs moteurs};
    \draw[ligne] (dg.south) -- (repartiteur.north);

    % Départ 1 — Ventilateur soufflage
    \node[depart, below left=1.0cm and 3.0cm of repartiteur] (d1)
      {$Q_1$ disj. moteur\\5,5\,kW / 10\,A\\+ Contacteur $KM_1$\\+ Relais th. $F_1$};
    \draw[ligne] (repartiteur.south west) ++(1.5,0) -- (d1.north);
    \node[font=\tiny, below=0.1cm of d1, color=FedGreen] {Vent. soufflage};

    % Départ 2 — Ventilateur reprise
    \node[depart, below=1.0cm of repartiteur] (d2)
      {$Q_2$ disj. moteur\\3,0\,kW / 6,3\,A\\+ Contacteur $KM_2$\\+ Relais th. $F_2$};
    \draw[ligne] (repartiteur.south) -- (d2.north);
    \node[font=\tiny, below=0.1cm of d2, color=FedGreen] {Vent. reprise};

    % Départ 3 — Pompe
    \node[depart, below right=1.0cm and 3.0cm of repartiteur] (d3)
      {$Q_3$ disj. moteur\\1,5\,kW / 3,2\,A\\+ Contacteur $KM_3$\\+ Relais th. $F_3$};
    \draw[ligne] (repartiteur.south east) ++(-1.5,0) -- (d3.north);
    \node[font=\tiny, below=0.1cm of d3, color=FedGreen] {Pompe batterie};

    % DDR
    \node[boite, fill=FedRed!10, right=1.2cm of dg] (ddr)
      {DDR 30\,mA\\Circuits\\commande};
    \draw[ligne] (jdb.south east) ++(-1.0,0) |- (ddr.north);
    \node[font=\tiny, below=0.1cm of ddr, color=FedRed] {Protection personnes};

  \end{tikzpicture}
  \caption{Schéma unifilaire simplifié d'une armoire électrique CVC triphasée 400\,V : disjoncteur général $Q_0$, trois départs moteurs avec disjoncteur moteur, contacteur et relais thermique, et un départ différentiel 30\,mA pour les circuits de commande.}
  \label{fig:armoire-cvc}
\end{figure}

\subsection{Courbes intensité-temps : fusible aM vs disjoncteur magnétothermique}

La figure~\ref{fig:courbes-protection} illustre les zones de déclenchement caractéristiques d'un fusible de type aM et d'un disjoncteur magnétothermique utilisés en protection moteur CVC.

\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[>=Stealth, thick, xscale=1.0, yscale=0.9]
    % Axes
    \draw[->] (0,0) -- (7.5,0) node[right, font=\small] {$I / I_n$};
    \draw[->] (0,0) -- (0,6.5) node[above, font=\small] {$t$ (s)};
    ...
    % Graduation axe Y
    \foreach \y/\lab in {1/10, 2/100, 3/1000, 4/3600, 5/36000} {
      \draw (-0.1, \y) -- (0.1, \y);
      \node[left, font=\tiny] at (-0.1, \y) {\lab};
    }
    \node[left, font=\tiny, rotate=90] at (-0.8, 3) {$t$ (s) — échelle log};

    % Courbe fusible aM (déclenchement tardif en surcharge, rapide en cc)
    % Zone de non-déclenchement (zone blanche sous la courbe)
    \draw[color=FedOrange, line width=2pt]
      (1.2, 5.5) .. controls (2.0, 5.0) and (3.0, 4.0) ..
      (4.5, 3.0) .. controls (5.5, 2.2) and (6.0, 1.5) .. (7.0, 0.8);
    \node[color=FedOrange, font=\small\bfseries, right] at (7.0, 0.9)
      {Fusible aM};

    % Fusible aM zone de tolérance (bande)
    \draw[color=FedOrange, dashed, line width=1pt]
      (1.0, 5.8) .. controls (2.0, 5.4) and (3.2, 4.3) ..
      (4.8, 3.3) .. controls (5.8, 2.5) and (6.3, 1.8) .. (7.2, 1.1);

    % Courbe disjoncteur magnétothermique
    % Zone thermique (inclinaison modérée)
    \draw[color=FedBlue, line width=2pt]
      (1.1, 3.8) .. controls (1.5, 3.5) and (2.0, 3.0) ..
      (3.0, 2.5) .. controls (3.8, 2.0) and (4.2, 1.5) .. (4.6, 1.2);
    % Déclenchement magnétique instantané (chute verticale)
    \draw[color=FedBlue, line width=2pt]
      (4.6, 1.2) -- (4.6, 0.05);
    \draw[color=FedBlue, line width=2pt]
      (4.6, 0.05) -- (7.2, 0.05);
    \node[color=FedBlue, font=\small\bfseries] at (3.2, 1.0)
      {Disjoncteur MT};
    % Flèche zone magnétique
    \draw[->, color=FedBlue, thin] (5.0, 0.6) -- (4.7, 0.1);
    \node[color=FedBlue, font=\tiny, right] at (5.0, 0.6) {zone magnétique};

    % Zone de démarrage moteur (pointe admissible)
    \fill[FedGreen!20] (1.0, 0) rectangle (4.0, 2.2);
    \draw[color=FedGreen, dashed] (1.0, 0) rectangle (4.0, 2.2);
    \node[color=FedGreen!70!black, font=\tiny, align=center] at (2.5, 1.1)
      {Zone démarrage\\moteur (5--8$\times I_n$)};

    % Légende
    \draw[color=FedOrange, line width=1.5pt] (0.3,6.2) -- (1.1,6.2);
    \node[right, font=\tiny, color=FedOrange] at (1.1,6.2)
      {Fusible aM — laisse passer la pointe de démarrage};
    \draw[color=FedBlue, line width=1.5pt] (0.3,5.8) -- (1.1,5.8);
    \node[right, font=\tiny, color=FedBlue] at (1.1,5.8)
      {Disjoncteur magnétothermique — protection surcharge + cc};
  \end{tikzpicture}
  \caption{Courbes intensité-temps (échelle log) : fusible aM (orange) vs disjoncteur magnétothermique (bleu). La zone verte représente la pointe de courant admissible au démarrage moteur ($5$ à $8 \times I_n$). Le fusible aM tolère cette pointe ; le déclencheur magnétique du disjoncteur est réglé au-delà de cette zone.}
  \label{fig:courbes-protection}
\end{figure}

\begin{tcolorbox}[
  enhanced, colback=FedBlue!3, colframe=FedBlue!50,
  fonttitle=\bfseries\sffamily, coltitle=FedBlue,
  title={FICHE MÉMO — Électrotechnique appliquée CVC}, boxrule=0.8pt, arc=2pt,
  width=\textwidth
]
\textbf{Puissances en triphasé équilibré :}
\[
  S = \sqrt{3}\,U\,I \;[\text{VA}] \qquad
  P = \sqrt{3}\,U\,I\,\cos\varphi \;[\text{W}] \qquad
  Q = \sqrt{3}\,U\,I\,\sin\varphi \;[\text{var}] \qquad
  S^2 = P^2 + Q^2
\]

\textbf{Facteur de puissance :}
\[
  \cos\varphi = \frac{P}{S} \quad [\text{sans unité}]
  \qquad \text{(objectif en conception GCF : } \cos\varphi \geq 0{,}85\text{)}
\]

\textbf{Vitesse de synchronisme et glissement (moteur asynchrone) :}
\[
  n_s = \frac{60\,f}{p} \;[\text{tr/min}]
  \qquad
  g = \frac{n_s - n}{n_s} \;[\text{typiquement } 2\text{ à }5\,\%]
  \quad f = 50\,\text{Hz},\; p = \text{nb de paires de pôles}
\]

\textbf{Lois de similitude (pompe / ventilateur centrifuge) :}
\[
...
\textbf{Correction du facteur de puissance par condensateurs :}
\[
  Q_C = P\,\bigl(\tan\varphi_1 - \tan\varphi_2\bigr) \;[\text{kvar}]
  \qquad P \;[\text{kW}]
\]

\textbf{Courant d'emploi (moteur triphasé) :}
\[
  I_B = \frac{P}{\sqrt{3}\,U\,\cos\varphi\,\cdot\,\eta} \;[\text{A}]
  \qquad P \;[\text{W}],\; U \;[\text{V}]
\]

\textbf{Chute de tension (circuit triphasé) :}
\[
  \Delta U = \frac{\sqrt{3}\,\rho\,L\,I}{S} \;[\text{V}]
  \qquad \rho_{\text{Cu}} \approx 0{,}0225\;\Omega\cdot\text{mm}^2/\text{m}, \quad
  \Delta U\,[\%] = \frac{\Delta U}{U} \times 100 \;\leq 3\,\%
\]
\end{tcolorbox}
