...
\begin{figure}[H]
  ...
  \node[box, fill=orange!15] (pac)   at (0,0)   {PAC\\Air/Eau\\20\,kW};
  ...
  \draw[aller] (pac.east)  -- node[above, font=\footnotesize] {40\,°C} (p1.west);
  ...
  % ---- Températures PAC ----
  \node[font=\footnotesize, text=red!70!black, above] at (1.5, 0.5) {départ 40\,°C};
  \node[font=\footnotesize, text=blue!70!black, below] at (1.5,-0.5) {retour 35\,°C};

  \end{tikzpicture}
  \caption{Schéma hydraulique simplifié — Immeuble résidentiel RE2020 :
    PAC air/eau, ballon tampon, distribution vers plancher chauffant (35/30\,\si{\celsius})
    et VMC double flux, avec robinets thermostatiques.}
\end{figure}
...
\begin{figure}[H]
  ...
  % ---- Locaux ----
  \node[comp, fill=green!10]  (loc)  at (13.2,-2.5) {Locaux\\26\,°C};
  ...
  \end{tikzpicture}
  \caption{Schéma aéraulique simplifié de la CTA double flux — Bâtiment tertiaire :
    trajet de l'air neuf (filtre G4, batterie chaude 70/60\,°C, batterie froide 7/12\,°C,
    humidificateur vapeur, ventilateur soufflage) et circuit de reprise (filtre M5,
    ventilateur de reprise). Les flèches rouges indiquent l'air chaud, les bleues l'air
    froid. Les états de l'air (température, humidité) sont annotés entre chaque composant.}
\end{figure}
...
\begin{figure}[H]
  ...
  % ---- Groupe froid ----
  \node[box, fill=blue!12]  (gf)   at (0,0)    {Groupe froid\\eau glacée\\246\,kW};
  ...
  \draw[recirc, -Latex] (salle.east) -- ++(1.0,0) -- ++(0, 1.8)
    ...
    {recirculation partielle (5\,vol/h)}
    |- (cta.north);

  \end{tikzpicture}
  \caption{Schéma de principe — Salle propre ISO\,7, installation industrielle pharmaceutique :
    groupe froid eau glacée alimentant la CTA (filtres F7 + HEPA H14),
    surpression de la salle ($+15$\,Pa) avec extraction filtrée,
    récupération de chaleur sur l'air extrait et recirculation partielle.}
\end{figure}
