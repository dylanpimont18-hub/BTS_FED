...
\begin{tikzpicture}[
  >=Stealth,
  bloc/.style={
    draw=FedBlue, fill=FedBlue!15,
    rounded corners=4pt,
    minimum width=2.6cm, minimum height=0.9cm,
    text centered, font=\small
  },
  comparateur/.style={
    draw=FedBlue, fill=white,
...
% Perturbation
\node[font=\footnotesize\itshape, color=FedRed] (pert) at (10.8,1.8) {Perturbation};
...
\draw[fl, dashed, color=FedRed] (pert) -- (proc.north);
...
% --- Schéma C — Courbes de réponse PID à un échelon ---
\begin{figure}[H]
\centering
\begin{tikzpicture}[x=0.7cm, y=0.9cm]
  % Axes
  \draw[->, thick] (0,0) -- (11,0) node[right, font=\small] {Temps $t$};
  \draw[->, thick] (0,0) -- (0,2.8) node[above, font=\small] {Grandeur réglée $y$};

  % Consigne (échelon à t=1)
  \draw[gray!50, dashed, thick] (1,0) -- (1,2.0) -- (10.5,2.0);
  \node[gray, font=\footnotesize, anchor=west] at (10.6,2.0) {Consigne $w$};
  \draw[gray!40, thin] (0,2.0) -- (1,2.0);
  \node[gray, font=\footnotesize, anchor=east] at (-0.1,2.0) {$w$};

  % Courbe P seule — erreur statique résiduelle
  \draw[FedOrange, thick, domain=1:10.5, samples=60]
    plot (\x, {2.0 * (1 - exp(-(\x-1)/1.8))  * 0.78 });
  \node[FedOrange, font=\scriptsize\bfseries, anchor=west] at (10.55, 1.55)
    {P (écart statique)};

  % Courbe PI — pas de dépassement ou léger
  \draw[FedGreen, thick, domain=1:10.5, samples=80]
    plot (\x, {2.0*(1 - exp(-(\x-1)/1.5)*(1 + (\x-1)/1.5))});
  \node[FedGreen, font=\scriptsize\bfseries, anchor=west] at (10.55, 1.93)
    {PI (sans écart)};

  % Courbe PID — dépassement + retour rapide
  \draw[FedBlue, thick, domain=1:10.5, samples=100]
    plot (\x, {2.0*(1 - exp(-(\x-1)/0.9)*( cos(deg(1.8*(\x-1))) + 0.55*sin(deg(1.8*(\x-1)))))});
  \node[FedBlue, font=\scriptsize\bfseries, anchor=west] at (10.55, 2.05)
    {PID (dépassement)};

  % Annotation dépassement
  \draw[FedBlue, thin, <->] (2.75,2.0) -- (2.75,2.42)
    node[midway, right, font=\scriptsize, FedBlue] {dép.};

  % Annotation temps de réponse PI (à 95%)
  \draw[FedGreen, thin, dotted] (0,1.9) -- (10.5,1.9);
  \draw[FedGreen, thin, <->] (1,0) -- (5.0,0)
    node[midway, below, font=\scriptsize, FedGreen] {$t_{95\%}$};
  \draw[FedGreen, thin, dotted] (5.0,0) -- (5.0,1.9);

  % Origine
  \node[font=\footnotesize] at (0,0) [below left] {0};
\end{tikzpicture}
\caption{Schéma C — Réponse à un échelon de consigne : comparaison P, PI et PID
         (dépassement, temps de réponse, erreur statique)}
\label{fig:reponse-pid}
\end{figure}
...
\begin{tikzpicture}[x=0.35cm, y=0.13cm]
  ...
  \draw[FedBlue, line width=1.8pt] (0,45) -- (28,5);
  \node[FedBlue, font=\footnotesize\bfseries, anchor=south west] at (0,45)
    {Radiateurs};

  % --- Courbe 2 : Plancher chauffant (pente douce) ---
  % Points : T_ext=-10,T_dep=45 → (0,20) ; T_ext=18,T_dep=28 → (28,3)
  \draw[FedOrange, line width=1.8pt, dashed] (0,20) -- (28,3);
  \node[FedOrange, font=\footnotesize\bfseries, anchor=north west] at (0,20)
    {Plancher chauffant};

  % --- Point de bivalence PAC ---
  % T_ext=-2 → x=8 ; T_dep=50 → y=25
  \filldraw[FedRed] (8,25) circle (0.6ex);
  \draw[FedRed, thin, ->, shorten >=2pt]
    (8,25) -- ++(5,6)
    node[anchor=south west, font=\footnotesize, color=FedRed,
         align=left]
  ...
  % --- Légende ---
  \draw[FedBlue, line width=1.5pt] (16,50) -- (20,50);
  \node[right, font=\footnotesize] at (20.3,50) {Radiateurs};
  \draw[FedOrange, line width=1.5pt, dashed] (16,46) -- (20,46);
...
\begin{tcolorbox}[
  ...
  title={FICHE MÉMO — Régulation et GTB}, boxrule=0.8pt, arc=2pt,
  width=\textwidth
]

\textbf{Correcteur proportionnel (P) :}
\[
  u(t) = K_P \cdot e(t) + u_0
  \qquad \text{avec} \quad X_P = \frac{1}{K_P} \times 100 \; [\si{\percent\per\kelvin}]
\]
$K_P$ = gain proportionnel, $e(t) = w - y(t)$ = erreur, $X_P$ = bande proportionnelle [\si{\kelvin}]

\medskip
\textbf{Correcteur PID complet :}
\[
  u(t) = K_P \left[ e(t) + \frac{1}{T_i} \int_0^t e(\tau)\,\mathrm{d}\tau + T_d \frac{\mathrm{d}e}{\mathrm{d}t} \right]
\]
$T_i$ = temps d'intégrale [\si{\second}] (annule l'écart statique) ; $T_d$ = temps dérivé [\si{\second}] (anticipe les variations)

\medskip
\textbf{Loi d'eau (régulation climatique) :}
\[
  T_{\text{dep}} = T_{\text{dep,base}} + K_{\text{loi}} \cdot (T_{\text{ext,base}} - T_{\text{ext}})
\]
$T_{\text{dep}}$ = température de départ [\si{\celsius}] ; $K_{\text{loi}}$ = pente de la courbe de chauffe (sans dimension)

\medskip
\textbf{Démarrage optimal (préchauffage) :}
\[
  t_{\text{start}} = t_{\text{occupation}} - \frac{w - T_{\text{amb,nuit}}}{K_{\text{bâtiment}} \times (T_{\text{dep,max}} - w)}
\]
$t_{\text{start}}$ = heure de démarrage [\si{\hour}] ; $K_{\text{bâtiment}}$ = constante de temps du bâtiment

\medskip
\textbf{Puissance PAC en fonction de la température extérieure :}
\[
  \dot{Q}_{\text{PAC}}(T_{\text{ext}}) = \dot{Q}_{\text{PAC,max}} \times \frac{T_{\text{ext}} - T_{\text{min}}}{T_{\text{max}} - T_{\text{min}}}
\]
$\dot{Q}_{\text{PAC,max}}$ = puissance maximale [\si{\kW}] ; $T_{\text{min}}$, $T_{\text{max}}$ = plage de fonctionnement [\si{\celsius}]

\medskip
\textbf{Niveaux d'architecture GTB :}

\begin{tabular}{@{}lll@{}}
  \toprule
  Niveau & Désignation & Exemples \\
  \midrule
  1 & Terrain & Capteurs, actionneurs, vannes, sondes \\
  2 & Automatisme & Automates, régulateurs locaux, UTA \\
  3 & Supervision & IHM, serveur GTB, tableaux de bord \\
  \bottomrule
\end{tabular}
