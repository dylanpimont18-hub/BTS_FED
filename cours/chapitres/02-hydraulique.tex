...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[thick, >=Stealth, font=\small]
    ...
    \pic at (0,-2.5)  {pompe};
    \pic at (2.5,1.0) {vase-expansion};
    \pic at (2.5,-3.8){radiateur};
    \pic at (5.5,-3.8){radiateur};
    \pic at (8.5,-3.8){radiateur};

    % ---- Nourrice (distributeur — rectangle standard) ----
    \node[draw=FedBlue, fill=FedBlue!10, rounded corners=2pt,
          minimum width=2.0cm, minimum height=0.75cm,
          font=\small\bfseries] (nourrice) at (6.0,0) {Nourrice};
    ...
    \node[font=\scriptsize\bfseries, text=FedGreen!70!black]
      at (2.5,1.72)  {Vase exp.};
    \node[font=\scriptsize, text=FedRed!80] at (2.5,-4.45) {Rad.~1};
    \node[font=\scriptsize, text=FedRed!80] at (5.5,-4.45) {Rad.~2};
    \node[font=\scriptsize, text=FedRed!80] at (8.5,-4.45) {Rad.~3};

    % ---- Départ chaudière → nourrice (rouge) ----
    \draw[->, FedRed, very thick]
      (0.55,0) -- node[above, font=\footnotesize, text=FedRed]{Départ 70°C}
      (nourrice.west);

    % ---- Vase d'expansion raccordé sur le départ ----
    \draw[FedGreen, thick] (2.5,0)--(2.5,0.62);

    % ---- Distribution nourrice → radiateurs (rouge) ----
    \draw[FedRed, thick] (nourrice.south)--(6.0,-2.5)--(2.5,-2.5);
    \draw[FedRed, thick] (6.0,-2.5)--(8.5,-2.5);
    \draw[->, FedRed, thick] (2.5,-2.5)--(2.5,-3.45);
    \draw[->, FedRed, thick] (5.5,-2.5)--(5.5,-3.45);
    \draw[->, FedRed, thick] (8.5,-2.5)--(8.5,-3.45);

    % ---- Retour radiateurs → collecteur → pompe (bleu) ----
    \draw[FedBlue, thick] (2.5,-4.15)--(2.5,-5.2);
    \draw[FedBlue, thick] (5.5,-4.15)--(5.5,-5.2);
    \draw[FedBlue, thick] (8.5,-4.15)--(8.5,-5.2);
    \draw[FedBlue, thick] (2.5,-5.2)--(8.5,-5.2);
    \draw[->, FedBlue, very thick]
      (5.5,-5.2)--(0,-5.2)--(0,-2.92);

    % ---- Pompe → chaudière (sortie pompe, retour) ----
    \draw[->, FedBlue, very thick]
      (0,-2.08) -- node[right, font=\footnotesize, text=FedBlue]{Retour 55°C}
      (0,-0.40);

    % ---- Légende ----
    \draw[FedRed,  very thick, ->] (1.8,-6.2)--++(0.7,0);
    \node[right, font=\footnotesize, text=FedRed]  at (2.5,-6.2) {Aller (eau chaude)};
    \draw[FedBlue, very thick, ->] (1.8,-6.7)--++(0.7,0);
    \node[right, font=\footnotesize, text=FedBlue] at (2.5,-6.7) {Retour (eau froide)};
    \node[font=\footnotesize, text=FedGreen!70!black]   at (7.0,-6.2)
      {Vase d'expansion — symbole ISO (cercle + membrane)};
    \node[font=\footnotesize, text=FedOrange!80!black]  at (7.0,-6.7)
      {Pompe de circulation — symbole ISO (cercle + impulseur)};

  \end{tikzpicture}
  \caption{Schéma de principe d'un circuit hydraulique bouclé avec symboles ISO GCF
           (chauffage bitubes)}
